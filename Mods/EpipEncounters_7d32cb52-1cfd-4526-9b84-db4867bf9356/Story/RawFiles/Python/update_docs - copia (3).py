...
DATA_MATCHERS = {
    re.compile("^---@param (?P<Type>\S*) (?P<Comment>.*)$"): Parameter,
    re.compile("^---@return (?P<Type>\S*) ?(?P<Comment>.*)$"): Return,
    re.compile("^---@field (?P<Type>\S*) ?(?P<Comment>.*)$"): ClassField,
    # re.compile("^---@return (?P<Comment>\S*)$"): Return,
    # re.compile("^---@meta (?P<Type>\S*) (?P<Comment>.*)$"): Return,
    re.compile("^---(?P<Comment>[^@].*)$"): Comment,
    re.compile("^---@meta (?P<Comment>.*)$"): Meta,
}

SYMBOL_MATCHERS = {
    FUNCTION_REGEX: Function,
    re.compile("^---@meta Library: (?P<Library>\S*), (?P<Context>\S*)$"): LibraryDefinition,
    re.compile("^---@class (?P<Class>\S*)$"): Class,
}
        ...
        for line in lines:
            ...
            if not hasSymbol or not symbol.isFinishedParsing(currentData):
                for matcher in DATA_MATCHERS:
                    ...
                    if match:
                        dataType = DATA_MATCHERS[matcher]
            ...
            for matcher in SYMBOL_MATCHERS:
                ...
                if match:
                    symbolType = SYMBOL_MATCHERS[matcher]

                    symbol = symbolType(None, [], match.groupdict())
